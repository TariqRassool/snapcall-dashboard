...
function renderAgents() {
  ...
  ge('agents-list').innerHTML = listAgents.length ? listAgents.map(a => {
    ...
    return `<div class="tmr"><div style="min-width:0;display:flex;align-items:center;gap:7px;overflow:hidden"><span style="font-weight:500;font-size:13px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;flex-shrink:1">${a.name}</span><span class="bdg ${(a.product || '').toLowerCase()}" style="flex-shrink:0">${prodLabel(a.product)}</span>${a.team ? '<span style="font-size:11px;color:var(--t3);white-space:nowrap;flex-shrink:0">· ' + a.team + '</span>' : ''}</div><div style="font-weight:500">${fmt(a.snaps)}</div><div><div class="rc" style="justify-content:center"><div class="rbg" style="width:50px"><div class="rbf" style="width:${Math.min(a.rate,100).toFixed(0)}%;background:${rc};height:3px"></div></div><span class="mono" style="color:${rc}">${a.rate.toFixed(1)}%</span></div></div><div style="color:var(--t2)">${a.csat > 0 ? a.csat.toFixed(2) + '/5' : '—'}</div><div>${initiatedBadge(a)}</div><div>${usageStyle(a)}</div></div>`;
